...
const InfoCard: React.FC<InfoCardProps> = ({ title, color, items, showCheckmarks = false }) => {
  ...
  const maxTilt = 2; // Subtle tilt for minimalistic glass effect
  const tiltX = mousePos ? ((mousePos.y - dimensions.height / 2) / (dimensions.height / 2)) * -maxTilt : 0;
  const tiltY = mousePos ? ((mousePos.x - dimensions.width / 2) / (dimensions.width / 2)) * maxTilt : 0;
  ...
  return (
    <div
      ref={cardRef}
      className={`flex-1 p-6 w-full min-h-[320px] bg-white bg-opacity-10 backdrop-blur-md rounded-none 
          hover:ring-[1px] ${theme === 'dark' ? 'hover:ring-neutral-800' : 'hover:ring-neutral-200'}
        hover:scale-100 transform-gpu transition-all duration-300 ease-in-out ${textColor}`}
      onMouseMove={handleMouseMove}
      onMouseLeave={handleMouseLeave}
      style={{
        transform: `perspective(1000px) rotateX(${tiltX}deg) rotateY(${tiltY}deg) scale(${mousePos ? 1.03 : 1})`,
        ...
        background: theme === 'dark' ? `linear-gradient(to bottom right, rgba(72, 72, 72, 0.1), rgba(72, 72, 72, 0.1)), ${glassHighlight}` : `linear-gradient(to bottom right, rgba(180, 180, 180, 0.1), rgba(180, 180, 180, 0.1)), ${glassHighlight}`,
      }}
    >
        ...
        {items.map((item) => (
          <div key={item.label}>
            <div className="flex items-center justify-between">
              <p className={`text-sm font-medium font-sans ${textColor} transition`}>{item.label}</p>
              {showCheckmarks && (
